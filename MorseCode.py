...
class CWMachine(StateMachine):
    ...
    def on_enter_dot(self):
        ...
        # blink(on_time, off_time, fade_in_time, fade_out_time, number of blinks)
        self.redLight.blink(on_time=0.5,off_time=1,n=1,background=False)
    ...
    def on_exit_dot(self):
        ...
        if(DEBUG):
            print("* Changing state red to off - dash")
    ##
    ...
    def on_enter_dash(self):
        ...
        # blink(on_time, off_time, fade_in_time, fade_out_time, number of blinks)
        self.blueLight.blink(on_time=1.5,off_time=1,n=1,background=False)
    ...
    def on_exit_dash(self):
        ...
        self.blueLight.off()
        if(DEBUG):
            print("* Changing state blue to off - dash")
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        if(DEBUG):
                            print(f"* Processing morse character: {x}")

                        ## Dot or dash?
                        ## Toggle the LED state to on then off for each dot then dash
                        if (x == '.'):
                            self.send("doDot")
                            self.send("doDot")
                        elif (x == '-'):
                ...
                if (wordsCounter < lenWords):
                    ...
                    # if done processing the word, pause before the message repeats
                    self.send("doLP")
                    self.send("doLP")

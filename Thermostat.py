...
##
## Import for the 7segmented display 
...
##
## Initialize the 7segmented display (extra)
## The second display has a jumper at A0 to change the address from 70 to 71 
## to use both displays in the series
##
segmentDisplay = segments.Seg7x4(i2c, address=0x70)
segmentDisplay2 = segments.Seg7x4(i2c, address=0x71)
...
class ManagedDisplay():
    ...
    def cleanupDisplay(self):
        ...
        ## Clear the 2 7-segmented displays
        segmentDisplay.fill(0)
        segmentDisplay2.fill(0)
...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_off(self):
        ...
        ## Reset the lights when the state is set to off
        redLight.off()
        blueLight.off()
    ...
    def processTempIncButton(self):
        ...
        ## Pressing both red and blue button will reset the 
        ## setPoint temp to the original
        if blueButton.is_pressed:
            self.setPoint = 72
        else:
            self.setPoint += 1

        ## update the leds depeding on what state the thermostat is in
        self.updateLights()
    ...
    def updateLights(self):
        ...
        ## Using a switch on state to determine what mode the thermostat is in
        match state:

            ## when the thermostat is set to heat
            case "heat":
                ## If the actual temp is lower than the set temp 
                ## pulse the red light
                if (temp < self.setPoint):
                    redLight.pulse()

                ## If the actual temp is the same of higher  
                ## keep the red light solid
                elif (temp >= self.setPoint):
                    redLight.on()

            ## when the thermostat is set to cool
            case "cool":
                ## If the actual temp is greater then set temp  
                ## pulse the blue light
                if (temp > self.setPoint):
                    blueLight.pulse()

                ## If the actual temp is the same of lower  
                ## keep the blue light solid
                elif (temp <= self.setPoint):
                    blueLight.on()
    ...
    def setupSerialOutput(self):
        ...
        ## Setting the output with the state, current temp and setpoint.
        ## Ends with a newline to tell the port the message is complete
        output = f"{self.current_state.id},{self.getFahrenheit():0.1f},{self.setPoint:0.1f}\n"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Setting the current state to uppercase for display
            mode = self.current_state.id.upper()

            ## Setup display line 1
            lcd_line_1 = current_time.strftime('%b %d  %H:%M:%S\n')

            ## Refresh the 7segmented display 
            ## Clear the display first then send the mode to one display
            ## Send the setpoint to the other display 
            segmentDisplay.fill(0)
            segmentDisplay.print(f"{mode}")
            segmentDisplay2.print(f"{self.setPoint:0.1f}F")

            ## Setup Display Line 2
            if(altCounter < 4):
                ## Display the current temp
                lcd_line_2 = f"Cur Temp : {self.getFahrenheit():0.1f}"
                altCounter = altCounter + 1
            else:
                ## Display the current mode and set temprature
                lcd_line_2 = f"{mode} set T:{self.setPoint:0.1f}"
            ...
            if((counter % 30) == 0):
                ...
                ## Write the mode, temp and settemp to the serial port in a comma delimited string
